...
extension UIColor {
    ...
    open class var defaultNavBarTintColor: UIColor {
        return UINavigationBar.appearance().barTintColor ?? UIColor.white
    }
}
...
extension UINavigationController {
    ...
    open class func initialization() {
    ...
    fileprivate func setNeedsNavigationBackground(alpha: CGFloat) {
        ...
        if navigationBar.isTranslucent {
            if #available(iOS 11.0, *) {
                ...
                self.navigationBar.shadowImage = alpha == 0 ? UIImage(): nil
               self.navigationBar.subviews.first?.alpha = alpha
                self.navigationBar.subviews.first?.subviews.forEach({ (view) in
                    view.alpha = alpha
                })
                self.navigationBar.subviews.first?.subviews[1].subviews.forEach({ (view) in
                    
                    view.alpha = alpha
                })
               // self.navigationBar.subviews.last?.alpha = alpha
            }else
